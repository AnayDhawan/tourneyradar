...
type Props = {
  ...
  title?: string;
  subtitle?: string;
  /** Called right after a successful submit, e.g. so a modal can close itself. */
  onSubmitted?: () => void;
};

// Shared star-rating + optional-comment + submit UI, used by both the
// dedicated /feedback page (linked from About) and the full-screen popup
// (FeedbackPrompt) so the actual insert-into-Supabase logic lives in one
// place rather than two.
export default function FeedbackForm({ fromPage, title, subtitle, onSubmitted }: Props) {
  ...
  }, [rating, submitting, comment, fromPage, user, userType, showToast, onSubmitted]);
